...
public class PlayingWithAlgo {
	...
	public static ArrayList<Integer> exportAllElementsPair(int[] array) {
		...
		for (int i = 0; i < array.length; i++) {
			if (array[i] % 2 == 0) {
				Integer nombre = (Integer) array[i];
				reponse.add(nombre);
			}
		}
		return reponse;
	...
	public static boolean checkSiTuPeuxAcheter(int prix) {
		// t'as 22 euros en monnaies de 2 euros, la machine ne rend pas de
		// monnaie, check si tu peux payer
		boolean achat = true;
		if ((prix > 22) || (prix % 2 != 0)) {
			achat = false;
		}
	...
	public static boolean checkCase3(int number1, int number2, int number3) {
		// check si les trois chiffres sont dans l'ordre, soit croissant ou
		// decroissant
		boolean reponse = false;
	...
	public static boolean checkIfStringStartsWithVowel(String word) {
		boolean reponse = false;
		String motMinuscules = word.toLowerCase ();
		char premiereLettre = motMinuscules.charAt(0);
		char voyelles [] = {'a' , 'e' , 'i' , 'o' , 'u' , 'y'};
		for (char test : voyelles) {
			if (premiereLettre == test) {
				reponse = true;
			}
		}
		return reponse;
